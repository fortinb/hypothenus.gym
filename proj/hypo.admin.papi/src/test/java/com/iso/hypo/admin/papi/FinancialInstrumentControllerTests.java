...
class FinancialInstrumentControllerTests {
	...
	void testPostSuccess(String role, String user) throws MalformedURLException, JsonProcessingException, Exception {
		...
		// Act
		FinancialInstrumentDto createdDto = 
	...
	void testPostFailureForbiddenBrandMismatch(String role, String user) throws MalformedURLException, JsonProcessingException, Exception {
		...
		ErrorDto _ = 
				this.restClient.post()
					.uri(HttpUtils.createURL(URI.create(String.format(postURI, faker.code().isbn10(), member.getUuid())), port, null))
					.headers(h -> h.addAll(HttpUtils.createHttpHeaders(role, user)))
					.accept(org.springframework.http.MediaType.APPLICATION_JSON)
					.contentType(org.springframework.http.MediaType.APPLICATION_JSON)
					.body(postDto)
	...
	void testActivateSuccess(String role, String user) throws JsonProcessingException, MalformedURLException {
		...
		// Act
		FinancialInstrumentDto activatedDto = 
				this.restClient.post()
					.uri(HttpUtils.createURL(URI.create(String.format(postActivateURI, brand.getUuid(), member.getUuid(), financialInstrumentToActivate.getUuid())), port, null))
					.headers(h -> h.addAll(HttpUtils.createHttpHeaders(role, user)))
					.accept(org.springframework.http.MediaType.APPLICATION_JSON)
					.contentType(org.springframework.http.MediaType.APPLICATION_JSON)
					.exchange() 
		...
		// Assert
		Assertions.assertEquals(true, activatedDto.isActive(), "Financial Instrument should be active");
		Assertions.assertNotNull(activatedDto.getActivatedOn(), "ActivationOn is null");
		assertFinancialInstrument(modelMapper.map(financialInstrumentToActivate, FinancialInstrumentDto.class), activatedDto);
	}
	...
	void testActivateFailureNotFound(String role, String user) throws JsonProcessingException, MalformedURLException {
		...
		ErrorDto _ = 
				this.restClient.post()
					.uri(HttpUtils.createURL(URI.create(String.format(postActivateURI, brand.getUuid(), member.getUuid(), faker.code().isbn10())), port, null))
					.headers(h -> h.addAll(HttpUtils.createHttpHeaders(role, user)))
					.accept(org.springframework.http.MediaType.APPLICATION_JSON)
					.contentType(org.springframework.http.MediaType.APPLICATION_JSON)
					.exchange() 
	...
	void testDeactivateSuccess(String role, String user) throws JsonProcessingException, MalformedURLException {
		...
		// Act
		FinancialInstrumentDto deactivatedDto = 
				this.restClient.post()
					.uri(HttpUtils.createURL(URI.create(String.format(postDeactivateURI, brand.getUuid(), member.getUuid(), financialInstrumentToDeactivate.getUuid())), port, null))
					.headers(h -> h.addAll(HttpUtils.createHttpHeaders(role, user)))
					.accept(org.springframework.http.MediaType.APPLICATION_JSON)
					.contentType(org.springframework.http.MediaType.APPLICATION_JSON)
					.exchange() 
		...
		// Assert
		Assertions.assertEquals(false, deactivatedDto.isActive(), "Financial Instrument should be inactive");
		Assertions.assertNotNull(deactivatedDto.getDeactivatedOn(), "DeactivationOn is null");
		assertFinancialInstrument(modelMapper.map(financialInstrumentToDeactivate, FinancialInstrumentDto.class), deactivatedDto);
	}
	...
	void testDeactivateFailureNotFound(String role, String user) throws JsonProcessingException, MalformedURLException {
		...
		ErrorDto _ = 
				this.restClient.post()
					.uri(HttpUtils.createURL(URI.create(String.format(postDeactivateURI, brand.getUuid(), member.getUuid(), faker.code().isbn10())),	port, null))
					.headers(h -> h.addAll(HttpUtils.createHttpHeaders(role, user)))
					.accept(org.springframework.http.MediaType.APPLICATION_JSON)
					.contentType(org.springframework.http.MediaType.APPLICATION_JSON)
					.exchange() 
	...
	void testDeleteSuccess() throws JsonProcessingException, MalformedURLException {
		...
		// Act
		FinancialInstrumentDto _ = 
				this.restClient.delete()
					.uri(HttpUtils.createURL(URI.create(String.format(deleteURI, brand.getUuid(), member.getUuid(),	financialInstrumentToDelete.getUuid())), port, null))
					.headers(h -> h.addAll(HttpUtils.createHttpHeaders(Roles.Admin, Users.Admin)))
					.accept(org.springframework.http.MediaType.APPLICATION_JSON)
					.exchange() 
